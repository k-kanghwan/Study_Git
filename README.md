...
- CLI 명령어(Git)
    ...
    | 구분                | Command                                                        | Description                                                     |
    | ------------------- | -------------------------------------------------------------- | --------------------------------------------------------------- |
    | 상태 확인           | status -s                                                      | 파일 변경 상태를 간략히 요약해 보여줌                           |
    | 커밋                | git commit -a                                                  | `git add` 명령어 생략 후 변경된 파일을 커밋                     |
    | 원격 작업           | git push [-u] [원격 저장소 별명] [브랜치 이름]                 | 로컬 변경 사항을 원격 저장소에 업로드                           |
    | 원격 작업           | git pull                                                       | `git fetch`와 `git merge`를 합쳐 원격 변경 사항을 가져오고 병합 |
    | 원격 작업           | git fetch [원격 저장소 별명] [브랜치 이름]                     | 원격 저장소의 변경 사항을 로컬 저장소로 가져옴                  |
    | 병합 작업           | git merge <대상 브랜치>                                        | 지정한 브랜치의 내용을 현재 브랜치에 병합                       |
    | 작업 취소           | git reset [파일명] ...                                         | 스테이징 취소 (mixed, soft, hard 옵션 사용 가능)                |
    | 로그 확인           | git log -n<숫자>                                               | 최근 n개의 커밋 로그를 표시                                     |
    | 도움말 확인         | git help <명령어>                                              | 특정 명령어의 도움말을 표시                                     |
    | 설정 확인           | git config --list                                              | Git 설정값 리스트를 확인                                        |
    | 설정 변경           | git config --global <옵션명>                                   | 전역 Git 설정에서 특정 옵션 값을 조회                           |
    | 설정 변경           | git config --global <옵션명> <새로운 값>                       | 전역 Git 설정에서 특정 옵션 값을 변경                           |
    | 설정 변경           | git config --global --unset <옵션명>                           | 전역 Git 설정에서 특정 옵션 값을 제거                           |
    | 원격 작업           | git remote add <원격 저장소 이름> <원격 저장소 주소>           | 새로운 원격 저장소를 추가                                       |
    | 복제                | git clone <저장소 주소> [새로운 폴더명]                        | 원격 저장소를 복제해 로컬에 저장                                |
    | 브랜치 관리         | git branch [-v]                                                | 브랜치 목록과 간략한 정보를 확인                                |
    | 브랜치 관리         | git branch [-f] <브랜치 이름> [커밋 체크섬]                    | 지정한 브랜치를 특정 커밋으로 강제로 이동                       |
    | 브랜치 전환         | git switch <브랜치 이름>                                       | 브랜치를 변경                                                   |
    | 브랜치 생성 및 전환 | git switch -c <브랜치 이름> [커밋 체크섬]                      | 새로운 브랜치를 생성하고 이동                                   |
    | 병합 작업           | git merge <대상 브랜치>                                        | 대상 브랜치의 커밋 기록을 현재 브랜치에 병합                    |
    | 리베이스            | git rebase <대상 브랜치>                                       | 대상 브랜치의 커밋 기록을 현재 브랜치에 재배치                  |
    | 리베이스            | git rebase --continue                                          | 충돌 해결 후 rebase를 계속 진행                                 |
    | 브랜치 삭제         | git branch -d <브랜치 이름>                                    | 병합된 브랜치를 삭제                                            |
    | 브랜치 삭제         | git branch -D <브랜치 이름>                                    | 병합 여부와 상관없이 브랜치를 강제로 삭제                       |
    | 작업 취소           | git reset --hard <이동할 커밋 체크섬/HEAD~2/HEAD^2>            | 변경 사항을 모두 취소하고 지정한 커밋으로 되돌아감              |
    | 태그 작업           | git tag -a -m <간단한 메시지> <태그 이름> [브랜치 또는 체크섬] | 지정한 브랜치 또는 커밋에 태그를 추가                           |
    | 태그 작업           | git push <원격 저장소 이름> <태그 이름>                        | 원격 저장소에 태그를 푸시                                       |
    | 병합 작업           | git merge --abort                                              | 충돌 발생 시 병합을 중단하고 병합 전 상태로 되돌리기            |
