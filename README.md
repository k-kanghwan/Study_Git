...
- CLI 명령어(Git)
    ...
    <!-- <깃 명령어>를 <예시 표>를 참고하여 description을 채워줘 -->
    | Command                                                        | Description                                                     |
    | -------------------------------------------------------------- | --------------------------------------------------------------- |
    | status -s                                                      | 파일 변경 상태를 간략히 요약해 보여줌                           |
    | git commit -a                                                  | `git add` 명령어 생략 후 변경된 파일을 커밋                     |
    | git push [-u] [원격 저장소 별명] [브랜치 이름]                 | 로컬 변경 사항을 원격 저장소에 업로드                           |
    | git pull                                                       | `git fetch`와 `git merge`를 합쳐 원격 변경 사항을 가져오고 병합 |
    | git fetch [원격 저장소 별명] [브랜치 이름]                     | 원격 저장소의 변경 사항을 로컬 저장소로 가져옴                  |
    | git merge <대상 브랜치>                                        | 지정한 브랜치의 내용을 현재 브랜치에 병합                       |
    | git reset [파일명] ...                                         | 스테이징 취소 (mixed, soft, hard 옵션 사용 가능)                |
    | git log -n<숫자>                                               | 최근 n개의 커밋 로그를 표시                                     |
    | git help <명령어>                                              | 특정 명령어의 도움말을 표시                                     |
    | git config --list                                              | Git 설정값 리스트를 확인                                        |
    | git config --global <옵션명>                                   | 전역 Git 설정에서 특정 옵션 값을 조회                           |
    | git config --global <옵션명> <새로운 값>                       | 전역 Git 설정에서 특정 옵션 값을 변경                           |
    | git config --global --unset <옵션명>                           | 전역 Git 설정에서 특정 옵션 값을 제거                           |
    | git remote add <원격 저장소 이름> <원격 저장소 주소>           | 새로운 원격 저장소를 추가                                       |
    | git clone <저장소 주소> [새로운 폴더명]                        | 원격 저장소를 복제해 로컬에 저장                                |
    | git branch [-v]                                                | 브랜치 목록과 간략한 정보를 확인                                |
    | git branch [-f] <브랜치 이름> [커밋 체크섬]                    | 지정한 브랜치를 특정 커밋으로 강제로 이동                       |
    | git switch <브랜치 이름>                                       | 브랜치를 변경                                                   |
    | git switch -c <브랜치 이름> [커밋 체크섬]                      | 새로운 브랜치를 생성하고 이동                                   |
    | git merge <대상 브랜치>                                        | 대상 브랜치의 커밋 기록을 현재 브랜치에 병합                    |
    | git rebase <대상 브랜치>                                       | 대상 브랜치의 커밋 기록을 현재 브랜치에 재배치                  |
    | git rebase <대상 브랜치>                                       | 대상 브랜치의 커밋 기록을 현재 브랜치에 재배치                  |
    | git branch -d <브랜치 이름>                                    | 병합된 브랜치를 삭제                                            |
    | git branch -D <브랜치 이름>                                    | 병합 여부와 상관없이 브랜치를 강제로 삭제                       |
    | git reset --hard <이동할 커밋 체크섬/HEAD~2/HEAD^2>            | 변경 사항을 모두 취소하고 지정한 커밋으로 되돌아감              |
    | git rebase <대상 브랜치>                                       | 대상 브랜치의 커밋을 현재 브랜치에 적용하고, 이전 커밋을 재배치 |
    | git rebase --continue                                          | 충돌 해결 후 rebase를 계속 진행                                 |
    | git tag -a -m <간단한 메시지> <태그 이름> [브랜치 또는 체크섬] | 지정한 브랜치 또는 커밋에 태그를 추가                           |
    | git push <원격 저장소 이름> <태그 이름>                        | 원격 저장소에 태그를 푸시                                       |
    | git merge --abort                                              | 충돌 발생 시 병합을 중단하고 병합 전 상태로 되돌리기            |
